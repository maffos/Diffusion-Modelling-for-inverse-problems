...
def grid_search(y_test, config, forward_model,forward_model_params,score_posterior,train,evaluate, train_args,eval_args):
    ...
    for param_configuration in utils.product_dict(**params):
        ...
        if param_configuration['metric'] == 'L1' and param_configuration['pde_loss'] == 'cScoreFPE':
            skip = True
        if loss_fn.name == 'DSM_PDE':
            if (params['lam'],params['metric']) in already_visited:
                skip = True
            else:
                already_visited.append((params['lam'],params['metric']))
        ...
        if not skip:
            ...
            if loss_fn.name == 'DSM_PDE':
                train_dir = os.path.join(config['src_dir'], param_configuration['pde_loss'], loss_fn.name,
                                         param_configuration['metric'], 'lam:{}'.format(param_configuration['lam']))
            else:
                train_dir = os.path.join(config['src_dir'], param_configuration['pde_loss'], loss_fn.name,
                                         param_configuration['metric'], param_configuration['ic_metric'],
            ...
            model = train(model, optimizer, loss_fn, forward_model,train_dir, log_dir, **train_args)
            kl,nlpd,fisher = evaluate(model, y_test, forward_model, out_dir, **eval_args)
            if (kl < best_kl):
