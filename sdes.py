"""
This module is taken from https://github.com/CW-Huang/sdeflow-light and slightly adapted to fit the purpose of inverse models.
"""

import torch
from include.sdeflow_light.lib.utils import sample_v, log_normal, sample_vp_truncated_q
...
class PluginReverseSDE(torch.nn.Module):
    ...
    # Drift
    def mu(self, t, x, cond,lmbd=0.):
        return (1. - 0.5 * lmbd) * self.base_sde.g(self.T-t, x) * self.a(x,cond, self.T - t) - \
               self.base_sde.f(self.T - t, x)

    #def mu(self, t, x, lmbd=0.):
    #    return (1. - 0.5 * lmbd) * self.base_sde.g(self.T-t, x) * self.a(x, self.T - t) - \
    #           self.base_sde.f(self.T - t, x)
